      ...
      >
        <div className="flex items-center gap-4 min-w-[1200px]">
          {steps.map((step, index) => {
            ...
            // Only prev and current steps are clickable
            return (
              <div key={step.number} className="flex-1 min-w-[220px]">
                ...
                <div
                  ...
                  `}
                  onClick={() => isClickable(step.number) && router.push(step.link)}
                  role={isClickable(step.number) ? "button" : undefined}
                  tabIndex={isClickable(step.number) ? 0 : -1}
                  onKeyDown={isClickable(step.number) ? (e) => {
